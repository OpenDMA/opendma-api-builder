...
public class SwiftPropertyImplementationFileWriter extends AbstractPropertyImplementationFileWriter
{
    ...
    protected void writeGenericSection(ApiDescription apiDescription, PrintWriter out) throws IOException
    {
        ...
        out.println("");
        out.println("    private static func isValidSingleValue(_ value: Any, for type: OdmaType) -> Bool {");
        ...
        while(itScalarTypes.hasNext())
        {
            ...
            out.println("        case ."+scalarTypeDescription.getName().toLowerCase()+": return value is "+(scalarTypeDescription.isReference() ? "OdmaObject" : apiWriter.getScalarDataType(scalarTypeDescription,false,true)));
        }
        out.println("        }");
        out.println("    }");
    }
    ...
    protected void writeSingleValueScalarAccess(ScalarTypeDescription scalarTypeDescription, PrintWriter out) throws IOException
    {
        ...
        out.println("    public func "+Tools.lowerCaseFirstChar(scalarName)+"Value() throws -> "+returnType+" {");
        String constantScalarTypeName = scalarTypeDescription.getName().toLowerCase();
        out.println("        guard !isMultiValue, type == ."+constantScalarTypeName+", let ret = value as? "+returnType+" else {");
    ...
    protected void writeMultiValueScalarAccess(ScalarTypeDescription scalarTypeDescription, PrintWriter out) throws IOException
    {
        ...
        out.println("    public func "+Tools.lowerCaseFirstChar(scalarName)+(scalarTypeDescription.isReference()?"Sequence":"Values")+"() throws -> "+returnType+" {");
        String constantScalarTypeName = scalarTypeDescription.getName().toLowerCase();
        out.println("        guard isMultiValue, type == ."+constantScalarTypeName+", let ret = value as? "+returnType+" else {");
