...
public class PhpPropertyImplementationFileWriter extends AbstractPropertyImplementationFileWriter
{
    ...
    protected void writeGenericSection(ApiDescription apiDescription, PrintWriter out) throws IOException
    {
        ...
        out.println("     */");
        out.println("    public function setValue($newValue): void");
        out.println("    {");
        ...
        out.println("                default =>  throw new OdmaRuntimeException(\"OdmaProperty initialized with unknown data type \".$this->dataType)");
        out.println("            }");
        out.println("        if ($this->multiValue) {");
        out.println("            if (!$this->checkListAndValues($newValue, $expectedType)) {");
        out.println("                throw new OdmaInvalidDataTypeException(\"This property has a multi-valued \".$this->dataType.\" data type. It can only be set to values assignable to \".$expectedType);");
        out.println("            }");
        out.println("        } else {");
        out.println("            if (!$newValue instanceof $expectedType) {");
        out.println("                throw new OdmaInvalidDataTypeException(\"This property has a single-valued \".$this->dataType.\" data type. It can only be set to values assignable to \".$expectedType);");
        out.println("            }");
        out.println("        }");
