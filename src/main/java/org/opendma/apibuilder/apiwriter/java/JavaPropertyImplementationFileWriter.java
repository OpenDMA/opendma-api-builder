...
public class JavaPropertyImplementationFileWriter extends AbstractPropertyImplementationFileWriter
{
    ...
    protected void writeGenericSectionSwitch(ApiDescription apiDescription, PrintWriter out, boolean multivalue) throws IOException
    {
        ...
        while(itScalarTypes.hasNext())
        {
            ...
            out.println("                } else {");
            String javaType = multivalue ? (scalarTypeDescription.isReference() ? "Iterable<OdmaObject>" : "List<"+apiWriter.getScalarDataType(scalarTypeDescription,false,false)+">") : (scalarTypeDescription.isReference() ? "OdmaObject" : apiWriter.getScalarDataType(scalarTypeDescription,false,false));
            out.println("                    throw new OdmaInvalidDataTypeException(\""+generatePropertyDataTypeDescription(multivalue, scalarTypeDescription)+". It can only be set to values assignable to `"+javaType+"`\");");
            out.println("                }");
            out.println("                break;");
        }
        out.println("            default:");
    ...
    protected void writeSingleValueScalarAccess(ScalarTypeDescription scalarTypeDescription, PrintWriter out) throws IOException
    {
        ...
        if(scalarTypeDescription.isReference())
		{
	        ...
	        out.println("        if( (multiValue == false) && (dataType == OdmaType."+constantScalarTypeName+") ) {");
	        out.println("            if(valueProvider == null) {");
            out.println("                if(value != null) {");
            out.println("                    if(value instanceof OdmaObject) {");
            out.println("                        return (("+stripAnnotation(javaReturnType)+")value).getId();");
            out.println("                    } else {");
            out.println("                        throw new OdmaRuntimeException(\"Internal error. Reference value is expected to be instance of OdmaObject\");");
            out.println("                    }");
            out.println("                } else {");
            out.println("                    return null;");
            out.println("                }");
	        out.println("            } else if(valueProvider.hasReferenceId()) {");
	        out.println("                return valueProvider.getReferenceId();");
	        out.println("            } else {");
	        out.println("                enforceValue();");
            out.println("                if(value != null) {");
            out.println("                    if(value instanceof OdmaObject) {");
